...
modFiles = list(filter(lambda f: os.path.isfile(os.path.join(modsFolder, f)) & f.endswith(".jar"), files))

for clientMod in modList:
    for modFile in modFiles:
        if clientMod.strip() in modFile:
            os.rename(os.path.join(modsFolder, modFile), os.path.join(modsFolder, f"{modFile}.disabled"))
            break
